...
class 整理又音():
            ...
            for row in 讀檔:
                ...
                if len(優勢音) == 2:
                    混合優勢音, 偏泉優勢音 = [
                        音.strip()
                        for 音 in 優勢音]
                    漢字, 混合優勢音, 混合優勢校對 = self.處理全部的漢字音標(對應表[row['主編碼']], 混合優勢音)
                    漢字, 偏泉優勢音, 偏泉優勢校對 = self.處理全部的漢字音標(對應表[row['主編碼']], 偏泉優勢音)
                    yield {
                        '文本資料': 漢字,
                        '種類': 字詞,
                    ...
                    漢字, 音標, 校對 = self.處理全部的漢字音標(對應表[row['主編碼']], row['又音'])
                    yield {
                        '文本資料': 漢字,
                        '種類': 字詞,
                        '屬性': {'音標': 音標},
    ...
    def 處理全部的漢字音標(self, 漢字, 音標):
        ...
        try:
            ...
            return 漢字, 正規化音標[1], [self.正規化漢字音標(校對漢字, 校對音標)]
